...
function startBruteForce() {
  ...
  const interval = setInterval(() => {
    ...
    statusText.innerHTML =
      `Tentative ${attempts}/${totalCombinations}: Test de ${currentAttempt}... Accès refusé!<br>` +
      statusText.innerHTML.split("<br>").slice(0, 4).join("<br>");
    ...
    if (progress >= 75 && Math.random() > 0.7) {
      ...
      statusText.innerHTML =
        `CONTOURNEMENT HYDRA RÉUSSI! Combinaison trouvée: ${correctCombination}<br>` +
        statusText.innerHTML;
